...
def main():
    ...
    glBufferData(GL_ARRAY_BUFFER, obj.model.itemsize *
                 len(obj.model), obj.model, GL_STATIC_DRAW)
    ...
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE,
                          obj.model.itemsize * 2, ctypes.c_void_p(texture_offset))
    glEnableVertexAttribArray(1)
    # normals
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE,
                          obj.model.itemsize * 3, ctypes.c_void_p(normal_offset))
        ...
        if glfw.get_mouse_button(window, 1) == 1:
            if position is None:
                if position2 is not None:
                    position = clef * \
                        glfw.get_cursor_pos(window)[0] - position2
                else:
